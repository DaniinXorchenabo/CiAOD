...
INLINE STATIC LPARRAY CALLBACK CreateArray(_In_ REGISTER CONST UINT64 lengthArray)
{
	...
	for (REGISTER UINT64 i = 0; i < lengthArray; ++i)
	{
		lpArray->data[i] = -1;
	}
...
INLINE STATIC VOID CALLBACK DeleteChains(_In_ REGISTER LPARRLPPNODE lpArrlppNode)
{
	for (REGISTER UINT64 i = 0; i < lpArrlppNode->length; ++i)
	{
	...
	_Out_opt_ REGISTER LPBOOL collision)
{
	...
	for (; lpNode->nextNode != NULL; lpNode = lpNode->nextNode);
	...
	_Out_opt_ REGISTER LPBOOL collision)
{
	...
	for (REGISTER UINT64 iter = hashCode; iter < lpArray->length; ++iter)
	{
...
INLINE STATIC UINT64 CALLBACK hash_0(_In_ REGISTER CONST INT64 keyValue, _In_ REGISTER CONST UINT64 length)
{
	return keyValue % length;
}

INLINE STATIC UINT64 CALLBACK hash_1(_In_ REGISTER CONST INT64 keyValue, _In_ REGISTER CONST UINT64 length)
{
	...
	for (REGISTER UINT64 i = length; i > 0; i /= 10, ++lm);
	--lm;
...
INLINE STATIC UINT64 CALLBACK hash_2(_In_ REGISTER CONST INT64 keyValue, _In_ REGISTER CONST UINT64 length)
{
	REGISTER UINT64 sum = 0;
	for (REGISTER UINT64 iter = keyValue; iter % length; sum += iter % length, iter /= length);

	return sum % length;
}

INLINE STATIC UINT64 CALLBACK hash_3(_In_ REGISTER CONST INT64 keyValue, _In_ REGISTER CONST UINT64 length)
{
	REGISTER CONST DOUBLE ka = keyValue * .6180339887498948;
	return (UINT64)(length * (ka - (INT64)ka));
}

STATIC UINT64(CALLBACK *CONST hashFunctions[])(_In_ REGISTER CONST INT64, _In_ REGISTER CONST UINT64) = { hash_0, hash_1, hash_2, hash_3 };
	...
	_Out_opt_ REGISTER PUINT64 comparisons)
{
	...
	for (REGISTER LPNODE iter = lpArrlppNode->lppNodes[hashFunction(keyValue, lpArrlppNode->length)];
		iter != NULL;
		iter = iter->nextNode)
	{
	...
	_Out_opt_ REGISTER PUINT64 comparisons)
{
	...
	for (REGISTER UINT64 iter = hashFunction(keyValue, lpArray->length);
		...
		case WM_COMMAND:
		{
			switch (LOWORD(wParam))
			{
				case BUTT_0:
				{
					...
					CONST INT iResultGetWindowText = GetWindowText(*outputFields, numberOfComparisonsStr, IOBUFFER);
					...
					for (UINT64 count = 0; count < numberOfComparisons; ++count)
					{
						CONST UINT64 arrNodeSize = 1000;
						LPARRLPPNODE lpArrlppNode[4];

						for (UINT64 iter = 0; iter < 4; ++iter)
						{
						...
						for (UINT64 iter = 0; iter < arrNodeSize; ++iter)
						{
							CONST INT64 rnd = rand() % 65000;
							for (UINT64 iter2 = 0; iter2 < 4; ++iter2)
							{
								BOOL coll;
								CONST BOOL bResultChains = ChainsMethod(lpArrlppNode[iter2], hashFunctions[iter2](rnd, arrNodeSize), rnd, &coll);
					...
					for (UINT64 i = 0; i < 4; ++i)
					{
						...
						CONST BOOL bResultSetWindowText = SetWindowText(outputFields[i + 1], outputBuffer);
				...
				case BUTT_1:
				{
					...
					for (UINT64 iter = 0; iter < hashTableLength; ++iter)
					{
						CONST INT64 keyValue = rand() % 10000;
						CONST UINT64 hashCode = hashFunctions[betterHashFunction](keyValue, hashTableLength);

						OpenAddressingMethod(lpArray, hashCode, keyValue, NULL);
						ChainsMethod(lpArrlppNode, hashCode, keyValue, NULL);
					}

					INT64 arrayKeys[10000];
					for (UINT64 iter = 0; iter < hashTableLength; ++iter)
					{
